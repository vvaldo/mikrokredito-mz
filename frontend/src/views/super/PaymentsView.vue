<template>
  <div class="modern-page">
    ...
    <div class="modern-card"><div class="table-head"><div><h2>Lista de pagamentos</h2><p class="muted">Pode exportar tudo ou apenas os pagamentos filtrados.</p></div><div class="action-row"><button class="btn" @click="exportExcel">Exportar Excel</button><button class="btn" @click="exportPdf">Exportar PDF</button></div></div>
      ...
      <table class="modern-table"><thead><tr><th>Referência</th><th>Cliente</th><th>Valor</th><th>Mora</th><th>Método</th><th>Submetido em</th><th>Registado por</th><th>Comprovativo</th><th>Estado</th><th></th></tr></thead><tbody><tr v-for="p in pagedPayments" :key="p.id"><td><strong>{{ p.reference }}</strong><br><span class="muted">{{ p.external_reference }}</span></td><td>{{ clientName(p) }}</td><td>{{ mzn(p.amount) }}</td><td>{{ mzn(p.applied_late_fee) }}</td><td>{{ methodLabel(p.method) }}</td><td>{{ dateTime(p.created_at) }}</td><td>{{ p.registered_by_user?.full_name || 'Sistema/Cliente' }}</td><td>{{ p.receipt_original_name || '—' }}</td><td><StatusBadge :status="p.status" /></td><td><div class="action-row"><button class="btn btn-sm" @click="viewPay(p)">Ver</button><button class="btn btn-sm" :disabled="!p.receipt_file_name" @click="downloadReceipt(p)">Ver comprovativo</button></div></td></tr><tr v-if="!filteredPayments.length"><td colspan="10" class="empty-state">Sem pagamentos para o filtro escolhido.</td></tr></tbody></table>
      </div>
...
const methodLabel=m=>({mpesa:'M-Pesa',emola:'e-Mola',bank_transfer:'Transferência bancária',cash:'Depósito/POS',reference:'Referência'})[m]||m; const dateTime=v=>v?new Date(v).toLocaleString('pt-MZ'):'—'; const clientName=p=>p.Loan?.LoanApplication?.Client?.User?.full_name || p.Loan?.Client?.User?.full_name || 'Cliente';
