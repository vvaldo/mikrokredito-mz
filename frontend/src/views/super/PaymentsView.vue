<template>
  <div class="modern-page">
    ...
    <div class="modern-card"><div class="table-head"><div><h2>Lista de pagamentos</h2><p class="muted">Pode exportar tudo ou apenas os pagamentos filtrados.</p></div><div class="action-row"><button class="btn" @click="exportExcel">Exportar Excel</button><button class="btn" @click="exportPdf">Exportar PDF</button></div></div>
      ...
      <table class="modern-table"><thead><tr><th>Referência</th><th>Cliente</th><th>Empréstimo</th><th>Valor</th><th>Mora</th><th>Método</th><th>Submetido em</th><th>Registado por</th><th>Comprovativo</th><th>Estado</th><th></th></tr></thead><tbody><tr v-for="p in pagedPayments" :key="p.id" class="clickable-row" tabindex="0" role="button" :aria-label="`Ver detalhe do pagamento ${p.reference}`" @click="viewPay(p)" @keydown.enter="viewPay(p)" @keydown.space.prevent="viewPay(p)"><td><strong>{{ p.reference }}</strong><br><span class="muted">{{ p.external_reference }}</span></td><td>{{ clientName(p) }}</td><td>{{ loanRef(p) }}</td><td>{{ mzn(p.amount) }}</td><td>{{ mzn(p.applied_late_fee) }}</td><td>{{ methodLabel(p.method) }}</td><td>{{ dateTime(p.created_at) }}</td><td>{{ p.registered_by_user?.full_name || 'Sistema/Cliente' }}</td><td><span :class="p.receipt_file_name?'badge-yesno-yes':'badge-yesno-no'">{{ p.receipt_file_name?'Sim':'Não' }}</span></td><td><StatusBadge :status="p.status" /></td><td><div class="action-row" @click.stop><button class="btn btn-sm" @click.stop="viewPay(p)">Ver</button><button class="btn btn-sm" :disabled="!p.receipt_file_name" @click.stop="downloadReceipt(p)">Ver comprovativo</button></div></td></tr><tr v-if="!filteredPayments.length"><td colspan="11" class="empty-state">Sem pagamentos para o filtro escolhido.</td></tr></tbody></table>
      </div>

      <div class="mobile-cards mobile-only">
        <div v-for="p in pagedPayments" :key="p.id" class="list-card" tabindex="0" role="button" @click="viewPay(p)" @keydown.enter="viewPay(p)" @keydown.space.prevent="viewPay(p)">
          <div class="list-card-head">
            <div><strong>{{ p.reference }}</strong><br><span class="muted">{{ clientName(p) }} · {{ loanRef(p) }}</span></div>
            <StatusBadge :status="p.status" />
          </div>
          <div class="loan-card-grid">
            <div><span class="muted">Valor</span><strong>{{ mzn(p.amount) }}</strong></div>
            <div><span class="muted">Mora</span><strong>{{ mzn(p.applied_late_fee) }}</strong></div>
            <div><span class="muted">Método</span><strong>{{ methodLabel(p.method) }}</strong></div>
            <div><span class="muted">Comprovativo</span><span :class="p.receipt_file_name?'badge-yesno-yes':'badge-yesno-no'">{{ p.receipt_file_name?'Sim':'Não' }}</span></div>
          </div>
          <div class="muted" style="font-size:11px;margin-bottom:8px">{{ dateTime(p.created_at) }} · {{ p.registered_by_user?.full_name || 'Sistema/Cliente' }}</div>
          <div class="action-row" @click.stop>
            <button class="btn btn-sm" @click.stop="viewPay(p)">Ver</button>
            <button class="btn btn-sm" :disabled="!p.receipt_file_name" @click.stop="downloadReceipt(p)">Ver comprovativo</button>
          </div>
        </div>
        <p v-if="!filteredPayments.length" class="empty-state">Sem pagamentos para o filtro escolhido.</p>
      </div>
...
<script setup>
import { ref, computed, onMounted, watch } from 'vue'; import { useToast } from 'vue-toastification'; import api, { downloadReceipt as downloadReceiptFile } from '@/services/api'; import StatusBadge from '@/components/common/StatusBadge.vue'; import AppPagination from '@/components/common/AppPagination.vue'; import LoadingSpinner from '@/components/common/LoadingSpinner.vue'
