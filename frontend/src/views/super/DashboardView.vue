<template>
  <div class="content-inner">
    <div class="page-header">
      <div><h1 class="page-title">Dashboard — Super Admin</h1><p class="page-sub">Números reais da base de dados · {{ lastRefresh || 'por actualizar' }}</p></div>
      <div class="page-actions"><button class="btn btn-primary" @click="load">Actualizar</button><button class="btn" @click="exportExcel">Exportar Excel</button></div>
    </div>

    <div v-if="loading" class="card">A carregar dados...</div>
    <template v-else>
      <div class="grid-4 mb-4">
        <div class="stat-card"><div class="stat-value blue">{{ stats.clients }}</div><div class="stat-label">Clientes</div></div>
      ...
      <div class="grid-4 mb-4">
        <div class="stat-card"><div class="stat-value">{{ stats.kyc_complete }}</div><div class="stat-label">KYC completo</div></div>
        <div class="stat-card"><div class="stat-value red">{{ stats.kyc_blocked }}</div><div class="stat-label">KYC bloqueado/incompleto</div></div>
        <div class="stat-card"><div class="stat-value">{{ mzn(stats.payments?.month) }}</div><div class="stat-label">Pagamentos no mês</div></div>
        <div class="stat-card"><div class="stat-value">{{ mzn(stats.payments?.year) }}</div><div class="stat-label">Pagamentos no ano</div></div>
      </div>

      <div class="grid-2">
        <div class="card">
          <div class="card-title">Pedidos recentes <button class="btn btn-sm" @click="router.push('/super/loans')">Ver todos</button></div>
          ...
          <div v-for="loan in recentApplications" :key="loan.id" class="dash-row">
            <div><div class="text-sm fw-medium">{{ loan.Client?.User?.full_name || 'Cliente' }}</div><div class="text-xs text-muted">{{ loan.reference }} · {{ mzn(loan.requested_amount) }} · {{ date(loan.created_at) }}</div></div>
            <div class="flex gap-1"><StatusBadge :status="loan.status" /><button class="btn btn-xs" @click="openLoan(loan)">Ver/Editar</button></div>
          </div>
        </div>
        <div class="card">
          <div class="card-title">Notificações recentes <button class="btn btn-sm" @click="router.push('/super/notifications')">Centro</button></div>
    ...
    <div v-if="modal" class="modal-backdrop" @click.self="modal=null">
      <div class="mk-modal wide"><div class="mk-modal-head"><h2>{{ modalTitle }}</h2><button class="modal-x" @click="modal=null">×</button></div>
        ...
        <div v-if="modal==='loan'" class="modal-actions">
          <button class="btn" @click="setStatus('under_review')">Colocar em análise</button>
          <button class="btn btn-primary" @click="setStatus('approved')">Aprovar</button>
          <button class="btn btn-danger" @click="setStatus('rejected')">Desaprovar</button>
          <button v-if="selected?.status==='approved'" class="btn btn-primary" @click="disburse">Desembolsar</button>
        </div>
      </div>
    </div>
  </div>
...
function openLoan(l){selected.value=l; modal.value='loan'; modalTitle.value='Pedido '+l.reference} function viewNotif(n){selected.value=n; modal.value='notif'; modalTitle.value='Log de notificação'}
...
async function disburse(){try{await api.post('/loans/'+selected.value.id+'/disburse'); toast.success('Desembolso registado na BD'); modal.value=null; await load()}catch(e){toast.error(e.response?.data?.message||'Erro ao desembolsar')}}
function exportExcel(){ router.push('/super/reports') }
onMounted(load)
</script>
