...
function accrueLateFee(schedule, rate, asOf) {
  const dueDate = new Date(schedule.due_date);
  const checkpoint = schedule.late_fee_accrued_through ? new Date(schedule.late_fee_accrued_through) : dueDate;
  const from = checkpoint > dueDate ? checkpoint : dueDate;
...
function applyPaymentToSchedules(schedules, amount, asOf, rate) {
  ...
  for (const schedule of ordered) {
    ...
    const accrual = accrueLateFee(schedule, rate, asOf);
...
function accrueOpenSchedules(schedules, rate, asOf) {
  return schedules.map(s => {
    const schedule = { ...s };
    if (!['pending', 'partial', 'overdue'].includes(schedule.status)) return schedule;
    if (new Date(schedule.due_date) >= startOfDay(asOf)) return schedule; // ainda não venceu
    const accrual = accrueLateFee(schedule, rate, asOf);
    schedule.late_fee = accrual.late_fee;
    schedule.late_fee_accrued_through = accrual.late_fee_accrued_through;
    const dueWithLate = round2(Number(schedule.total_due || 0) + Number(schedule.late_fee || 0));
...
function replayLoan(pristineSchedules, transactions, rate, today = new Date()) {
  ...
  for (const tx of sorted) {
    ...
    const { schedules: after, allocations, unallocated } = applyPaymentToSchedules(schedules, tx.amount, asOf, rate);
    schedules = after;
    allocationsByTx[tx.id] = allocations;
    if (unallocated > 0) unallocatedByTx[tx.id] = unallocated;
  }

  schedules = accrueOpenSchedules(schedules, rate, today);
